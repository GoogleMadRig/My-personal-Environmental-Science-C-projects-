...
int main() {
    ...
    for (int i = 0; i < room_number; i++) {
        ...
        cin >> input_choice;
        cin.ignore(numeric_limits<streamsize>::max(), '\n'); // This will ignore characters up to the end of line
        if (input_choice == 'k') {
            cout << "Please enter the room name: ";
           getline(cin,device.Room_name);
            cout<<"Your room name is "<<device.Room_name<<endl;
